        ...
        for topic, data in tqdm(topic_summaries.items(), desc="Embedding topics"):
            ...
            for attempt in range(1, retry_attempts + 1):
                try:
                    ...
                    logger.info(f"Successfully created embedding for topic: {topic}")
                    break  # Exit retry loop on success
                except RateLimitError as e:
                    logger.warning(f"Rate limit error on attempt {attempt}/{retry_attempts} for topic '{topic}': {e}")
                    if attempt < retry_attempts:
                        sleep_time = 2 ** attempt  # Exponential backoff
                        logger.info(f"Retrying after {sleep_time} seconds...")
                        time.sleep(sleep_time)
                    else:
                        logger.error(f"Max retry attempts reached for topic '{topic}'. Skipping embedding.")
                        embeddings[topic] = None
                except OpenAIError as e:
                    logger.error(f"OpenAI API error for topic '{topic}': {e}")
                    embeddings[topic] = None
...
def query_with_rag(question, embeddings):
    ...
    relevant_timeframes = [
        (embeddings[topic]["start_time"], embeddings[topic]["end_time"])
        for topic in top_topics if embeddings.get(topic)
    ]
